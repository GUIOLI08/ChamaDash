    ...
    try:
        ...
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            ...
            fmt_ans_ruim = workbook.add_format({
                "border": 1, "align": "center", "valign": "vcenter",
                "num_format": "0%", "font_color": "#C0504D", "bold": True,
            })
            ...
            for tipo_glpi, tipo_nome in tipos_para_tabela:
                if col_tipo and col_sla:
                    ...
                    totais_gerais["qtd"] += qtd_total; totais_gerais["baixa_no"] += b_no; totais_gerais["baixa_fora"] += b_fora
                    totais_gerais["media_no"] += m_no; totais_gerais["media_fora"] += m_fora; totais_gerais["alta_no"] += a_no; totais_gerais["alta_fora"] += a_fora
            ...
            if col_tipo:
                ...
                g_tipo.add_series({
                    "categories": ["Motor_Oculto", 0, 0, max(0, len(counts) - 1), 0],
                    "values": ["Motor_Oculto", 0, 1, max(0, len(counts) - 1), 1],
            ...
            if col_tipo and "Prio_Agrupada" in tabela.columns:
                ...
                g_inc.add_series({
                    "categories": ["Motor_Oculto", 0, 3, max(0, len(counts_inc) - 1), 3],
                    "values": ["Motor_Oculto", 0, 4, max(0, len(counts_inc) - 1), 4],
                    "data_labels": {"percentage": True},
                })
                ...
                g_req.add_series({
                    "categories": ["Motor_Oculto", 0, 6, max(0, len(counts_req) - 1), 6],
                    "values": ["Motor_Oculto", 0, 7, max(0, len(counts_req) - 1), 7],
                    "data_labels": {"percentage": True},
                })
            ...
            if "Categoria" in tabela.columns:
                ...
                desenhar_tabela(aba_dash, 31, 2, 3, "🏆 TOP 10 CATEGORIAS", "Categoria", cat_counts)
                aba_motor.write_column("J1", cat_counts.index); aba_motor.write_column("K1", cat_counts.values)
                g_cat = workbook.add_chart({"type": "bar"})
                g_cat.add_series({
                    "categories": ["Motor_Oculto", 0, 9, max(0, len(cat_counts) - 1), 9],
                    "values": ["Motor_Oculto", 0, 10, max(0, len(cat_counts) - 1), 10],
                    "fill": {"color": "#C0504D"},
                })
                g_cat.set_title({"name": "Gráfico - Top Categorias"})
                g_cat.set_legend({"none": True}); g_cat.set_style(10); g_cat.set_chartarea({"border": {"none": True}})
            ...
            if "Localização" in tabela.columns:
                setor_counts = tabela["Localização"].dropna().value_counts().head(10)
                desenhar_tabela(aba_dash, 31, 12, 3, "🏢 TOP 10 SETORES", "Setor", setor_counts)
                aba_motor.write_column("M1", setor_counts.index); aba_motor.write_column("N1", setor_counts.values)
                g_setor = workbook.add_chart({"type": "bar"})
                g_setor.add_series({
                    "categories": ["Motor_Oculto", 0, 12, max(0, len(setor_counts) - 1), 12],
                    "values": ["Motor_Oculto", 0, 13, max(0, len(setor_counts) - 1), 13],
                    "fill": {"color": "#4F81BD"},
                })
                g_setor.set_title({"name": "Gráfico - Top Setores"})
                g_setor.set_legend({"none": True}); g_setor.set_style(10); g_setor.set_chartarea({"border": {"none": True}})
            ...
            if col_tecnico:
                todos_tec_series = tabela[col_tecnico].dropna().value_counts() 
                desenhar_tabela(aba_dash, linha_s3-1, 6, 3, '🛠️ TÉCNICOS (TODOS)', 'Técnico', todos_tec_series)
                todos_tec = todos_tec_series.to_dict()

            col_grupo = 'Atribuído para - Grupo técnico'
            if col_grupo in tabela.columns:
                grupos = tabela[col_grupo].dropna().value_counts().head(8)
                aba_motor.write_column('P1', grupos.index); aba_motor.write_column('Q1', grupos.values)
                g_grupo = workbook.add_chart({'type': 'pie'})
                g_grupo.add_series({'categories': ['Motor_Oculto', 0, 15, max(0, len(grupos)-1), 15], 'values': ['Motor_Oculto', 0, 16, max(0, len(grupos)-1), 16], 'data_labels': {'percentage': True}})
            ...
            if col_sla:
                sla = tabela[col_sla].dropna().value_counts()
                aba_motor.write_column('S1', sla.index); aba_motor.write_column('T1', sla.values)
                g_sla = workbook.add_chart({'type': 'pie'})
                g_sla.add_series({'categories': ['Motor_Oculto', 0, 18, max(0, len(sla)-1), 18], 'values': ['Motor_Oculto', 0, 19, max(0, len(sla)-1), 19], 'data_labels': {'percentage': True}})
            ...
            sla_final = {"No Prazo": int(t_no_total), "Fora do Prazo": int(t_fora_total)}
