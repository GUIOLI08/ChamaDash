        ...
        if coluna_id in tabela.columns:
            ...
            for col in tabela.columns:
                if col == coluna_id:
                    ...
                    funcoes_de_agrupamento[col] = lambda x: '\n---\n'.join(pd.Series(x).dropna().astype(str).unique())
                else:
                    funcoes_de_agrupamento[col] = 'first'
                    
            tabela = tabela.groupby(coluna_id, as_index=False).agg(funcoes_de_agrupamento)

        print("Building Premium Dashboard...")
        output = io.BytesIO()
        
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            tabela.to_excel(writer, index=False, sheet_name='dados_formatados')
            workbook = writer.book
            aba_dados = writer.sheets['dados_formatados']
            
            formato_texto_quebrado = workbook.add_format({'text_wrap': True, 'valign': 'top'})
            for col_idx, col_name in enumerate(tabela.columns):
                if not tabela.empty:
                    tamanho_maximo = max(tabela[col_name].astype(str).map(len).max(), len(str(col_name)))
                else:
            ...
            def desenhar_tabela(aba, linha_ini, col_ini, col_span, titulo_geral, titulo_col1, serie_dados):
                ...
                if col_span > 1:
                    aba.merge_range(linha_ini+1, col_ini, linha_ini+1, col_fim_nome, titulo_col1, fmt_cabecalho_sub)
                else:
                ...
                for index, valor in serie_dados.items():
                    if col_span > 1:
                        aba.merge_range(l_atual, col_ini, l_atual, col_fim_nome, str(index), fmt_cel_esq)
                    else:
...
if __name__ == "__main__":
    ...
    print("Server running at http://[IP_ADDRESS]")
