...
# Configurando para servir a página HTML
templates = Jinja2Templates(directory="templates")

# Configurando para servir arquivos estáticos
app.mount("/static", StaticFiles(directory="static"), name="static")

def limpar_texto_glpi(texto):
    if pd.isna(texto):
        return texto
    texto = str(texto)
    texto = html.unescape(texto) # Transforma &aacute; em á, &ordm; em º
    texto = re.sub(r'<[^>]+>', ' ', texto) # Remove lixo de HTML como <p>, <div>, <br>
    texto = re.sub(r'\s+', ' ', texto).strip() # Tira espaços duplos e arruma a formatação
    return texto

# Rota principal para mostrar a página HTML
@app.get("/", response_class=HTMLResponse)
async def initial_page(request: Request):
    return templates.TemplateResponse("index.html", {"request": request})

# Rota POST para receber o arquivo
@app.post("/upload")
async def upload_file(archive: UploadFile = File(...)):
    
    try:
        if not archive:
            raise ValueError("No archive was received")
        ...
        print("\n• The archive has been received successfully!")
        print(f"• Archive name: {name}")
        print(f"• Archive size: {size_bytes} bytes")
        print("• Next step: excel magic will happen now")

        # Primeiro tentará ler como XLSX
        # Caso falhe tentará ler como XLS
        # Caso falhe tentará ler como CSV/SLK
        # Por fim, caso falhe forçará a leitura como Excel
        try:
            if name.endswith('.xlsx'):
                ...
                for linha_texto in texto_slk.splitlines():
                    if linha_texto.startswith('C;'):
                        match_y = re.search(r'Y(\d+)', linha_texto)
                        match_x = re.search(r'X(\d+)', linha_texto)
                        match_k = re.search(r'K"?(.*?)"?(?:;|$)', linha_texto) 
                        
                        if match_y: linha_atual = int(match_y.group(1))
                        if match_x: col_atual = int(match_x.group(1))
                        
                        if match_k:
                            val = match_k.group(1)
                            if linha_atual not in dados_slk:
                                dados_slk[linha_atual] = {}
                            dados_slk[linha_atual][col_atual] = val
                ...
                print('SLK was read successfully!')
                
            else:
                tabela = pd.read_csv(io.BytesIO(content), sep=';', encoding='latin-1', engine='python', on_bad_lines='skip')
                
        except Exception as e:
            print(f"Error reading the archive: {e}")
            raise ValueError(f"Error reading the archive: {e}")
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            workbook.worksheets_objs.insert(0, workbook.worksheets_objs.pop(workbook.worksheets_objs.index(aba_dash)))
            aba_dash.hide_gridlines(2)

            # Formatações Visuais
            formato_titulo = workbook.add_format({'bold': True, 'font_size': 22, 'font_color': '#1f497d'})
            fmt_cabecalho_top = workbook.add_format({'bold': True, 'bg_color': '#244062', 'font_color': 'white', 'border': 1, 'align': 'center', 'valign': 'vcenter'})
            fmt_cabecalho_sub = workbook.add_format({'bold': True, 'bg_color': '#4F81BD', 'font_color': 'white', 'border': 1, 'align': 'center', 'valign': 'vcenter'})
            fmt_cel_centro = workbook.add_format({'border': 1, 'align': 'center', 'valign': 'vcenter'})
            fmt_cel_esq = workbook.add_format({'border': 1, 'align': 'left', 'valign': 'vcenter'})
            fmt_ans = workbook.add_format({'border': 1, 'align': 'center', 'valign': 'vcenter', 'num_format': '0%'}) 
            fmt_ans_ruim = workbook.add_format({'border': 1, 'align': 'center', 'valign': 'vcenter', 'num_format': '0%', 'font_color': '#C0504D', 'bold': True})
            ...
            aba_dash.write('C2', 'Visão Geral de Chamados - Painel Executivo', formato_titulo)

            # A Matriz de Dados Principal
            linha_inicio = 6 
            col_matriz = 2
            
            aba_dash.merge_range(linha_inicio-2, col_matriz, linha_inicio-2, col_matriz+12, '📊 MATRIZ GERAL DE CHAMADOS E DESEMPENHO (SLA)', fmt_cabecalho_top)
            ...
            colunas_sub = ['Tipos', 'Qtd', 'No Prazo', 'Fora', 'ANS', 'No Prazo', 'Fora', 'ANS', 'No Prazo', 'Fora', 'ANS', 'No Prazo', 'Fora']
            ...
            col_sla = 'Tempo para solução excedido' if 'Tempo para solução excedido' in tabela.columns else None

            tipos_para_tabela = [('Incidente', 'Incidentes'), ('Requisição', 'Solicitações'), ('Problema', 'Problemas')]
            linha_dados = linha_sub + 1
            totais_gerais = {'qtd': 0, 'baixa_no': 0, 'baixa_fora': 0, 'media_no': 0, 'media_fora': 0, 'alta_no': 0, 'alta_fora': 0}
            ...
            for tipo_glpi, tipo_nome in tipos_para_tabela:
                if col_tipo and col_sla:
                    ...
                    def calc_prio(df, prioridade):
                        ...
                        ans = (no_prazo / (no_prazo + fora)) if (no_prazo + fora) > 0 else 1.0
                    ...
                    totais_gerais['baixa_no'] += b_no; totais_gerais['baixa_fora'] += b_fora
                    totais_gerais['media_no'] += m_no; totais_gerais['media_fora'] += m_fora
                    totais_gerais['alta_no'] += a_no; totais_gerais['alta_fora'] += a_fora
            ...
            linha_total = ['Total', totais_gerais['qtd'], t_b_no, t_b_fora, t_b_ans, t_m_no, t_m_fora, t_m_ans, t_a_no, t_a_fora, t_a_ans, t_b_no + t_m_no + t_a_no, t_b_fora + t_m_fora + t_a_fora]
            ...
            linha_s1 = 14
            if col_tipo:
                tipos_gerais = tabela[col_tipo].value_counts()
                aba_motor.write_column('A1', tipos_gerais.index); aba_motor.write_column('B1', tipos_gerais.values)
                g_tipo = workbook.add_chart({'type': 'pie'})
                g_tipo.add_series({'categories': ['Motor_Oculto', 0, 0, len(tipos_gerais)-1, 0], 'values': ['Motor_Oculto', 0, 1, len(tipos_gerais)-1, 1], 'data_labels': {'percentage': True}})
                g_tipo.set_title({'name': '1. Distribuição por Tipo'})
                g_tipo.set_style(10); g_tipo.set_chartarea({'border': {'none': True}})
                g_tipo.set_size({'width': 380, 'height': 260})
                aba_dash.insert_chart(f'C{linha_s1}', g_tipo, {'x_offset': 5})

            if col_tipo and 'Prio_Agrupada' in tabela.columns:
                inc_prio = tabela[tabela[col_tipo] == 'Incidente']['Prio_Agrupada'].value_counts()
                aba_motor.write_column('D1', inc_prio.index); aba_motor.write_column('E1', inc_prio.values)
                g_inc = workbook.add_chart({'type': 'pie'})
                g_inc.add_series({'categories': ['Motor_Oculto', 0, 3, len(inc_prio)-1, 3], 'values': ['Motor_Oculto', 0, 4, len(inc_prio)-1, 4], 'data_labels': {'percentage': True}})
                g_inc.set_title({'name': '2. Incidentes por Prioridade'})
                g_inc.set_style(10); g_inc.set_chartarea({'border': {'none': True}})
                g_inc.set_size({'width': 380, 'height': 260})
                aba_dash.insert_chart(f'H{linha_s1}', g_inc, {'x_offset': 5})

                req_prio = tabela[tabela[col_tipo] == 'Requisição']['Prio_Agrupada'].value_counts()
                aba_motor.write_column('G1', req_prio.index); aba_motor.write_column('H1', req_prio.values)
                g_req = workbook.add_chart({'type': 'pie'})
                g_req.add_series({'categories': ['Motor_Oculto', 0, 6, len(req_prio)-1, 6], 'values': ['Motor_Oculto', 0, 7, len(req_prio)-1, 7], 'data_labels': {'percentage': True}})
                g_req.set_title({'name': '3. Solicitações por Prioridade'})
                g_req.set_style(10); g_req.set_chartarea({'border': {'none': True}})
                g_req.set_size({'width': 380, 'height': 260})
                aba_dash.insert_chart(f'M{linha_s1}', g_req, {'x_offset': 5})

            # Categorias e Setores
            linha_s2 = 32
            if 'Categoria' in tabela.columns:
                top_cat = tabela['Categoria'].value_counts().head(10)
                desenhar_tabela(aba_dash, linha_s2-1, 2, 3, '🏆 TOP 10 CATEGORIAS', 'Categoria', top_cat) 
                aba_motor.write_column('J1', top_cat.index); aba_motor.write_column('K1', top_cat.values)
                g_cat = workbook.add_chart({'type': 'bar'})
                g_cat.add_series({'categories': ['Motor_Oculto', 0, 9, len(top_cat)-1, 9], 'values': ['Motor_Oculto', 0, 10, len(top_cat)-1, 10], 'fill': {'color': '#C0504D'}})
                g_cat.set_title({'name': 'Gráfico - Top Categorias'})
                g_cat.set_legend({'none': True})
                g_cat.set_style(10); g_cat.set_chartarea({'border': {'none': True}})
                g_cat.set_size({'width': 450, 'height': 300})
                aba_dash.insert_chart(f'G{linha_s2}', g_cat, {'x_offset': 15, 'y_offset': 5})

            if 'Localização' in tabela.columns:
                top_setor = tabela['Localização'].value_counts().head(10)
                desenhar_tabela(aba_dash, linha_s2-1, 12, 3, '🏢 TOP 10 SETORES', 'Setor', top_setor) 
                aba_motor.write_column('M1', top_setor.index); aba_motor.write_column('N1', top_setor.values)
                g_setor = workbook.add_chart({'type': 'bar'})
                g_setor.add_series({'categories': ['Motor_Oculto', 0, 12, len(top_setor)-1, 12], 'values': ['Motor_Oculto', 0, 13, len(top_setor)-1, 13], 'fill': {'color': '#4F81BD'}})
                g_setor.set_title({'name': 'Gráfico - Top Setores'})
                g_setor.set_legend({'none': True})
                g_setor.set_style(10); g_setor.set_chartarea({'border': {'none': True}})
                g_setor.set_size({'width': 450, 'height': 300})
                aba_dash.insert_chart(f'Q{linha_s2}', g_setor, {'x_offset': 15, 'y_offset': 5}) 

            # Usuários, Técnicos e SLA
            linha_s3 = 54
        ...
        print(str(err) or "Error processing the archive")
